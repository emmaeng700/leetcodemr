...
if MODE_NC_EXTRA:
    ...
    OUTPUT_PDF      = SCRIPT_DIR / ('the_digest.pdf' if CHAPTER2_PDF else 'simplified.pdf')
    OUTPUT_1UP      = SCRIPT_DIR / 'simplified_1up.pdf'
...
def _load_mode_data() -> tuple[list, dict, list]:
    """
    Load questions and doocs for the current mode (NC150 / AM600 / main).
    Returns (questions, doocs, rounds).
    """
...
if __name__ == '__main__':
    ...
    questions, doocs, rounds = _load_mode_data()
    print(f'  {len(questions)} questions · doocs: {len(doocs)}')
