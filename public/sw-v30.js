...
function isUsableImageResponse(res, path) {
  ...
  if (url.pathname.startsWith('/question-images/') || url.pathname.startsWith('/description-images/')) {
    e.respondWith(
      caches.open(IMG_CACHE).then(async imgCache => {
        ...
        if (cached && isUsableImageResponse(cached, url.pathname)) return cached
        if (cached) {
          try { await imgCache.delete(url.pathname, opts) } catch {}
        }
        try {
          const res = await fetch(e.request)
          if (isUsableImageResponse(res, url.pathname)) {
    ...
    e.waitUntil(
      caches.open(IMG_CACHE).then(async imgCache => {
        ...
        for (let i = 0; i < paths.length; i += batch) {
          ...
          await Promise.all(
            slice.map(async path => {
              try {
                ...
                if (existing && isUsableImageResponse(existing, path)) return
                if (existing) {
                  try { await imgCache.delete(path, opts) } catch {}
                }
          ...
          done = Math.min(i + batch, paths.length)
