...
Fits 9 mini-pages per printed sheet (landscape orientation).
...
Usage:
  python3 generate_patterns_print9up_landscape.py
Output:
...
INNER_PDF   = SCRIPT_DIR / "_bp_inner_9up_landscape.pdf"
OUTPUT_PDF  = SCRIPT_DIR / "LeetMastery_By_Pattern_Python_Only_Print_9up_Landscape.pdf"

# ─── Mini-page dimensions (9-up landscape: 3 cols × 3 rows on 792×612 sheet) ───
MP_W  = 792.0 / 3   # 264 pts  (wider mini-page)
MP_H  = 612.0 / 3   # 204 pts  (shorter mini-page)
MG    = 8.0
USE_W = MP_W - 2 * MG - 12  # 236 pts (RL SimpleDocTemplate adds ~12pt internal offset)
...
# ─── Code panel (colored syntax, dark background — native mini-page size) ─────
# No line clipping — chunk into groups of 10 so even fully-wrapped lines
# (worst case 10 × 2 × 7.5pt = 150pt + 8pt padding = 158pt) fit in the
# ~231pt frame height with comfortable margin.
_CODE_CHUNK = 10
...
def build_inner_pdf(groups: list, sites: dict, doocs: dict):
    ...
    story.append(Paragraph("Python Only · Print Edition · 9-up Landscape", ParagraphStyle(
        "sub2", fontName="LG-Bold", fontSize=9, textColor=BLACK,
        alignment=TA_CENTER, leading=12)))
    story.append(Spacer(1, 8))
    story.append(hr())
    story.append(Spacer(1, 5))
    story.append(Paragraph(
        f"{total_qs} questions · {len(groups)} patterns · bold black · 9-up landscape",
    ...
    for rank, (pat, qs) in enumerate(groups, 1):
        ...
        row = Table([[
            ...
            Paragraph(diff_str, ParagraphStyle(
                "dc", fontName="LG-Bold", fontSize=6, textColor=BLACK, alignment=TA_LEFT)),
...
def impose_9up_landscape(src_path: Path, dst_path: Path):
    ...
    for pg_idx in range(num_sheets):
        dst[pg_idx].insert_text(
            ...
            f"Sheet {pg_idx + 1} / {num_sheets}  ·  LeetMastery By Pattern · Python Only · 9-up Landscape",
