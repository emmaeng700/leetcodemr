...
export function listDropdownMobilePanelClasses(smAlign: 'left' | 'right') {
  ...
  return [
    'fixed left-1/2 top-[calc(56px+3.25rem)] z-[100]',
    'max-h-[min(70vh,22rem)] w-[min(calc(100vw-2rem),20rem)] -translate-x-1/2',
    'overflow-y-auto overscroll-contain rounded-xl border border-gray-200 bg-white shadow-xl',
...
export const listDropdownMobilePanelViewportOnly =
  'fixed left-1/2 top-[calc(56px+3.25rem)] z-[100] max-h-[min(70vh,22rem)] w-[min(calc(100vw-2rem),20rem)] -translate-x-1/2 overflow-y-auto overscroll-contain rounded-xl border border-gray-200 bg-white shadow-xl'
