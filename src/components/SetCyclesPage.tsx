...
export default function SetCyclesPage({ set }: Props) {
  const router = useRouter()
  const learnBase = set === 2 ? '/learn2' : '/learn3'
  const idxKey    = set === 2 ? 'lm_learn2_idx' : 'lm_learn3_idx'

  const [cycles,   setCycles]   = useState<SetSavedCycle[]>([])
  const [questions, setQs]      = useState<SetQuestion[]>([])
  ...
  const resolveRange = (): { start: number; end: number; label: string } | null => {
    ...
    const to   = parseInt(formTo || String(questions.length), 10) - 1
    if (!Number.isFinite(from) || !Number.isFinite(to) || from < 0 || to < from) return null
    const s = Math.max(0, Math.min(from, questions.length - 1))
    const e = Math.max(s, Math.min(to,   questions.length - 1))
  ...
  const handleDelete = (id: string) => {
    ...
    if (isActive) {
      ...
      toast.success(`"${cycle.name}" deleted and deactivated`)
    } else {
      toast.success(`"${cycle.name}" deleted`)
    }
  }

  const handleActivate = (cycle: SetSavedCycle) => {
    const active = getSetCycleState(set)
      ...
      {showForm && (
        <div className="bg-white border border-indigo-200 rounded-2xl p-5 mb-5 shadow-sm">
          ...
          {!formPreset && (
            <div className="flex items-center gap-2 mb-4">
              <div className="flex-1">
                ...
                <input type="number" min={1} max={questions.length} value={formFrom}
              ...
              <div className="flex-1">
                ...
                <input type="number" min={1} max={questions.length} value={formTo}
                  onChange={e => setFormTo(e.target.value)} placeholder={String(questions.length)}
                  className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-800 outline-none focus:border-indigo-400" />
              </div>
            </div>
          )}
          ...
          {(() => {
            ...
            const first = questions[rng.start]?.title ?? '?'
            const last  = questions[rng.end]?.title ?? '?'
            return (
              <p className="text-xs text-indigo-600 bg-indigo-50 rounded-xl px-3 py-2 mb-4">
                <strong>{size} questions</strong> · <em>{first}</em> → <em>{last}</em>
              </p>
            )
          })()}
