...
function filterSummary(filters: GrindFilterState): string {
  const parts: string[] = []
  if (filters.difficulties.size < 3) parts.push([...filters.difficulties].join('/') || 'none')
  if (filters.priorities.size < 3) parts.push([...filters.priorities].join('/') || 'none')
  if (filters.sets.size < 3) parts.push([...filters.sets].map(s => `S${s}`).join('/') || 'none')
  if (filters.pattern !== 'all') parts.push(filters.pattern)
...
export default function GrindCountStrip({
  ...
  const [open, setOpen] = useState(anyCustom)

  return (
    <div className="grind-count-strip" role="group" aria-label="Grind filters">
      <button
        type="button"
        className={`grind-filter-toggle ${anyCustom ? 'active' : ''}`}
        aria-expanded={open}
        onClick={() => setOpen(v => !v)}
      >
        <span>Filters</span>
        <span className="grind-filter-toggle-summary">{filterSummary(filters)}</span>
        <span className="grind-filter-toggle-caret" aria-hidden>{open ? '▴' : '▾'}</span>
      </button>

      <div className={`grind-filter-panel ${open ? 'open' : ''}`}>
        <div className="grind-filter-wrap">
          {DIFFS.map(d => {
          ...
          <span className="grind-filter-sep" aria-hidden>|</span>
          {PRIOS.map(p => {
        ...
        {!compact && (
          <div className="grind-filter-wrap grind-filter-pattern-row">
...
export function grindQuestionMatchesFilters(
  q: { id: number; set: 1 | 2 | 3; difficulty: string; pattern: string | null },
  filters: GrindFilterState,
): boolean {
