    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 mb-8">
        {[
          ...
          { label: 'Total', value: totalQ, color: 'text-[var(--text-muted)]', bg: 'bg-[var(--bg-muted)] border-[var(--border)]' },
          ...
          <div key={c.label} className={`rounded-xl border p-3 sm:p-5 flex sm:block items-center gap-3 sm:text-center ${c.bg}`}>
            ...
            <div className="text-xs text-[var(--text-subtle)] sm:mt-1 font-medium">{c.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border)] shadow-sm p-5 mb-6">
        ...
        <div className="w-full h-4 bg-[var(--bg-muted)] rounded-full overflow-hidden">
      ...
      {/* By difficulty */}
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border)] shadow-sm p-5 mb-6">
        <h2 className="font-bold text-[var(--text-muted)] text-sm mb-4 flex items-center gap-2">
            ...
            return (
              <div key={diff}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs text-[var(--text-subtle)]">{data.solved} / {data.total}</span>
                </div>
      ...
      {solvedList.length > 0 && (
        <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border)] shadow-sm p-5 mb-6">
          <h2 className="font-bold text-[var(--text-muted)] text-sm mb-3">✅ Solved Questions</h2>
            ...
            {solvedList.map(q => (
              <div key={q.id} className="flex items-center justify-between gap-2 text-sm min-w-0">
                <span className="text-[var(--text-muted)] truncate min-w-0">
                  <span className="text-[var(--text-subtle)] font-mono text-xs mr-2">#{q.id}</span>
                  {q.title}
                </span>
      ...
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border)] shadow-sm p-5 mb-6">
        <h2 className="font-bold text-[var(--text)] text-sm mb-1 flex items-center gap-2">🚔 LeetCode Police — Daily Target</h2>
        ...
        {planPerDay > 0 ? (
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[var(--text)] text-sm font-semibold">questions / day</p>
          ...
          <>
            {dailyTarget === 0 && (
              <>
                <p className="text-xs text-[var(--text-subtle)] mb-4">No study plan active. Commit to a daily goal and lock it with a code.</p>
                <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:items-end">
                  <div>
                    <label className="block text-xs text-[var(--text-subtle)] mb-1">Questions / day</label>
                    <input type="number" min="1" max="20" value={targetInput} onChange={e => setTargetInput(e.target.value)} placeholder="e.g. 3"
                      className="w-full sm:w-24 px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-muted)] text-[var(--text)] text-sm focus:outline-none focus:border-indigo-400" />
                  </div>
                  <div>
                    <label className="block text-xs text-[var(--text-subtle)] mb-1">Lock code</label>
                    <input type="text" value={lockCodeInput} onChange={e => setLockCodeInput(e.target.value)} placeholder="e.g. grind2026"
                      className="w-full sm:w-40 px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-muted)] text-[var(--text)] text-sm focus:outline-none focus:border-indigo-400" />
                  </div>
                ...
                <p className="text-xs text-[var(--text-subtle)] mt-2">Remember your code — there is no recovery if you forget it.</p>
              </>
            )}
            ...
            {dailyTarget > 0 && !isUnlocked && (
              <>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <p className="text-[var(--text)] text-sm font-semibold">questions / day 🔒</p>
                    <p className="text-xs text-[var(--text-subtle)]">Goal is locked. Enter your code to change it.</p>
                  </div>
                </div>
                <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:items-center">
                  <input type="text" value={unlockAttempt}
                    ...
                    className={`w-full sm:w-44 px-3 py-2 rounded-lg border bg-[var(--bg-muted)] text-[var(--text)] text-sm focus:outline-none transition-colors ${unlockError ? 'border-red-500' : 'border-[var(--border)] focus:border-indigo-400'}`} />
                ...
                <div className="mt-3 flex flex-wrap gap-3 text-xs text-[var(--text-subtle)]">
            ...
            {dailyTarget > 0 && isUnlocked && (
              <>
                ...
                <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:items-end mb-3">
                  <div>
                    <label className="block text-xs text-[var(--text-subtle)] mb-1">New questions / day</label>
                    <input type="number" min="1" max="20" value={targetInput} onChange={e => setTargetInput(e.target.value)}
                      className="w-full sm:w-24 px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-muted)] text-[var(--text)] text-sm focus:outline-none focus:border-indigo-400" />
                  </div>
                  <div>
                    <label className="block text-xs text-[var(--text-subtle)] mb-1">New lock code</label>
      ...
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border)] shadow-sm p-5 mb-6">
        <h2 className="font-bold text-[var(--text-muted)] text-sm mb-1 flex items-center gap-2">🔥 Activity Heatmap</h2>
        <p className="text-xs text-[var(--text-subtle)] mb-4">Any question solved that day shows green</p>
      ...
      {totalTime > 0 && (
        <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border)] shadow-sm p-5 mb-6">
          <h2 className="font-bold text-[var(--text-muted)] text-sm mb-3 flex items-center gap-2">⏱ Time Spent</h2>
          <div className="text-3xl font-black text-indigo-600 mb-1">{totalTime}m</div>
          <p className="text-xs text-[var(--text-subtle)]">total practice time across {Object.keys(timeData).length} questions</p>
        </div>
      )}
