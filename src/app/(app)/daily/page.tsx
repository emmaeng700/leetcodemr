...
const DailyContent  = dynamic(() => import('./DailyContent'),          { ssr: false })
const SpeedsterPage = dynamic(() => import('../speedster/page'),       { ssr: false })

const TABS = [
  { key: 'daily',     label: 'Daily',     icon: Calendar, Page: DailyContent  },
  { key: 'speedster', label: 'Speedster', icon: Gauge,    Page: SpeedsterPage },
]

function DailyInner() {
  const searchParams = useSearchParams()
  const router = useRouter()
  const [active, setActive] = useState(searchParams.get('tab') ?? 'daily')

  const go = (key: string) => {
    setActive(key)
    router.replace(`/daily?tab=${key}`, { scroll: false })
  }
