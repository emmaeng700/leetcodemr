...
function Learn3Hub() {
  ...
  useEffect(() => {
    ...
    router.replace(`/learn3/${Math.max(0, idx)}`)
  }, [tab, router])
    ...
    <div className="flex flex-col min-h-[calc(100dvh-56px)]">
      <div className="flex overflow-x-auto scrollbar-none border-b border-[var(--border)] bg-[var(--bg-card)] shrink-0">
        {TABS.map(({ key, label, icon: Icon }) => (
          <button key={key}
            onClick={() => router.replace(`/learn3?tab=${key}`, { scroll: false })}
      ...
      <div className="flex-1 min-h-0 overflow-auto">
        {tab === 'cycles' && <SetCyclesPage set={3} />}
      </div>
    </div>
  )
}
...
export default function Learn3Page() {
  return <Suspense><Learn3Hub /></Suspense>
}
