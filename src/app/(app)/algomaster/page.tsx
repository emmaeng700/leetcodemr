...
export default function AlgoMaster600Page() {
  ...
  const totalSolved = useMemo(
    () => ALGOMASTER_600.reduce((acc, cat) => acc + cat.questions.filter(q => solved.has(q.id)).length, 0),
    [solved]
  )

  const allQuestions = useMemo(() => ALGOMASTER_600.flatMap(c => c.questions), [])
  const easySolved = useMemo(() => allQuestions.filter(q => q.difficulty === 'Easy'   && solved.has(q.id)).length, [allQuestions, solved])
  const medSolved  = useMemo(() => allQuestions.filter(q => q.difficulty === 'Medium' && solved.has(q.id)).length, [allQuestions, solved])
  const hardSolved = useMemo(() => allQuestions.filter(q => q.difficulty === 'Hard'   && solved.has(q.id)).length, [allQuestions, solved])
  const easyTotal  = useMemo(() => allQuestions.filter(q => q.difficulty === 'Easy').length, [allQuestions])
        ...
        <div className="bg-[#16213e] border border-gray-700/50 rounded-2xl p-5 mb-6">
          <div className="flex items-end justify-between mb-3">
            <div>
              ...
              <span className="text-gray-500 text-sm font-medium ml-1">/ {TOTAL} solved</span>
            </div>
            <span className="text-xs text-gray-500">{loading ? '' : `${Math.round(totalSolved / TOTAL * 100)}% complete`}</span>
          </div>
