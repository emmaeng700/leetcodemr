"""
LeetMastery — By Pattern · Python Only · Print 9-up Portrait
=============================================================
Native 204×264 mini-pages imposed 3×3 on portrait letter — no scaling,
bold LG-Bold/Menlo-Bold, all font sizes +1 pt vs the 6-up edition.
Fits 9 mini-pages per printed sheet (portrait orientation).
...
Usage:
  python3 generate_patterns_print9up_portrait.py
Output:
  LeetMastery_By_Pattern_Python_Only_Print_9up_Portrait.pdf
"""
...
INNER_PDF   = SCRIPT_DIR / "_bp_inner_9up_portrait.pdf"
OUTPUT_PDF  = SCRIPT_DIR / "LeetMastery_By_Pattern_Python_Only_Print_9up_Portrait.pdf"

# ─── Mini-page dimensions (9-up portrait: 3 cols × 3 rows on 612×792 sheet) ───
MP_W  = 612.0 / 3   # 204 pts
MP_H  = 792.0 / 3   # 264 pts
MG    = 8.0
USE_W = MP_W - 2 * MG - 12  # 176 pts (RL SimpleDocTemplate adds ~12pt internal offset)
USE_H = MP_H - 2 * MG       # 248 pts
...
def build_inner_pdf(groups: list, sites: dict, doocs: dict):
    ...
    story.append(Paragraph("Python Only · Print Edition · 9-up Portrait", ParagraphStyle(
    ...
    story.append(Paragraph(
        f"{total_qs} questions · {len(groups)} patterns · bold black · 9-up portrait",
...
# ─── 9-up portrait imposer ────────────────────────────────────────────────────
def impose_9up_portrait(src_path: Path, dst_path: Path):
    ...
    for pg_idx in range(num_sheets):
        dst[pg_idx].insert_text(
            ...
            f"Sheet {pg_idx + 1} / {num_sheets}  ·  LeetMastery By Pattern · Python Only · 9-up Portrait",
            fontsize=6, color=(0.5, 0.5, 0.5),
        )

    dst.save(str(dst_path), garbage=4, deflate=True)
    src.close(); dst.close()
    print(f"9-up portrait: {n} mini-pages → {num_sheets} sheets → {dst_path.name}")
...
if __name__ == "__main__":
    ...
    print("Imposing 9-up portrait…")
    impose_9up_portrait(INNER_PDF, OUTPUT_PDF)
