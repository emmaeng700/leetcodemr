"""
LeetMastery — By Pattern · Python Only · Print 9-up Landscape
==============================================================
Native 264×204 mini-pages imposed 3×3 on landscape letter — no scaling,
bold LG-Bold/Menlo-Bold, all font sizes +1 pt vs the 6-up edition.
Fits 9 mini-pages per printed sheet.

Content per question (same as the 6-up By-Pattern PDF):
...
Usage:
  python3 generate_patterns_print9up.py
Output:
  LeetMastery_By_Pattern_Python_Only_Print_9up_Landscape.pdf
"""
...
INNER_PDF   = SCRIPT_DIR / "_bp_inner_9up.pdf"
OUTPUT_PDF  = SCRIPT_DIR / "LeetMastery_By_Pattern_Python_Only_Print_9up_Landscape.pdf"

# ─── Mini-page dimensions (9-up: 3 cols × 3 rows) ─────────────────────────────
MP_W  = 792.0 / 3   # 264 pts
MP_H  = 612.0 / 3   # 204 pts
MG    = 8.0
USE_W = MP_W - 2 * MG   # 248 pts
USE_H = MP_H - 2 * MG   # 188 pts
...
S = {
    "title":       ParagraphStyle("ttl", fontName="LG-Bold",    fontSize=9,   textColor=BLACK,    leading=11,  spaceAfter=1),
    "body":        ParagraphStyle("bd",  fontName="LG-Bold",    fontSize=7,   textColor=GRAY_700, leading=9,   spaceAfter=1),
    "body_sm":     ParagraphStyle("bds", fontName="LG-Bold",    fontSize=6.8, textColor=GRAY_700, leading=8.5, spaceAfter=1),
    "code":        ParagraphStyle("cd",  fontName="Menlo-Bold",  fontSize=6.5, textColor=BLACK,    leading=8.5),
    "head2":       ParagraphStyle("h2",  fontName="LG-Bold",    fontSize=7.5, textColor=BLACK,    leading=10,  spaceAfter=1),
    "toc":         ParagraphStyle("tc",  fontName="LG-Bold",    fontSize=8,   textColor=BLACK,    leading=11),
    "cover_title": ParagraphStyle("ct",  fontName="LG-Bold",    fontSize=14,  textColor=BLACK,    alignment=TA_CENTER, leading=17),
    "cover_sub":   ParagraphStyle("cs",  fontName="LG-Bold",    fontSize=8,   textColor=GRAY_700, alignment=TA_CENTER, leading=11),
}
...
def desc_to_mini_flowables(desc_html: str) -> list:
    if not desc_html:
        return []
    body_st = ParagraphStyle("dbody", fontName="LG-Bold", fontSize=7,   textColor=GRAY_700, leading=9,   spaceAfter=1)
    li_st   = ParagraphStyle("dli",   fontName="LG-Bold", fontSize=6.8, textColor=GRAY_700, leading=8.5, leftIndent=8, spaceAfter=1)
    hdr_st  = ParagraphStyle("dhdr",  fontName="LG-Bold", fontSize=7.5, textColor=BLACK,    leading=10,  spaceAfter=1, spaceBefore=3)
    pre_st  = ParagraphStyle("dpre",  fontName="Menlo-Bold", fontSize=6, textColor=BLACK,    leading=8)
    ...
    for m in block_re.finditer(desc_html):
        ...
        if m.group(4) is not None:
            ...
            if raw:
                ...
                lines = [ln[:65] for ln in safe.split("\n")[:8]]
...
# ─── Code panel ───────────────────────────────────────────────────────────────
_CODE_MAX_CHARS = 58  # clip long lines to prevent wrapping at 6.5pt Menlo on 238pt paragraph

def code_panel(code: str) -> list:
    if not code.strip():
        return []
    raw_lines = code.split("\n")
    clipped   = [ln[:_CODE_MAX_CHARS] + ("…" if len(ln) > _CODE_MAX_CHARS else "") for ln in raw_lines]
    xml_lines = [indent_xml(ln) for ln in clipped]
    items = []
    for i in range(0, len(xml_lines), 14):
        cell = Paragraph("<br/>".join(xml_lines[i:i+14]), S["code"])
        tbl  = Table([[cell]], colWidths=[USE_W])
        tbl.setStyle(TableStyle([
...
def cat_bar(text: str) -> Table:
    tbl = Table([[Paragraph(
        f"<b>{safe_xml(text)}</b>",
        ParagraphStyle("cb", fontName="LG-Bold", fontSize=6.5, textColor=BLACK),
    )]], colWidths=[USE_W])
    tbl.setStyle(TableStyle([
        ("BACKGROUND",    (0,0), (-1,-1), GRAY_100),
...
def site_label(label: str) -> Paragraph:
    return Paragraph(
        f"<b>● {label}</b>",
        ParagraphStyle(f"sl_{label}", fontName="LG-Bold", fontSize=6.5,
                       textColor=GRAY_700, leading=8, spaceBefore=3))
...
def build_question_block(q: dict, sites_cache: dict, doocs_cache: dict,
    ...
    items.append(Paragraph(links, ParagraphStyle(
        "lnk", fontName="LG-Bold", fontSize=6.5, textColor=BLACK, leading=9, spaceAfter=2)))
    ...
    if tags:
        items.append(Paragraph(
            ...
            ParagraphStyle("tg", fontName="LG-Bold", fontSize=6.5,
                           textColor=GRAY_500, leading=8, spaceAfter=2)))
    ...
    if source:
        items.append(Paragraph(
            ...
            ParagraphStyle("src", fontName="LG-Bold", fontSize=6.5,
                           textColor=GRAY_500, leading=8, spaceAfter=2)))
    ...
    if not is_js_pattern:
        ...
        if bf and bf.strip():
            items.append(Paragraph(
                ...
                ParagraphStyle("bfl", fontName="LG-Bold", fontSize=7,
    ...
    if is_js_pattern:
        ...
        for site_key, site_label_str in SITE_META:
            ...
            if not has_any:
                ...
                items.append(Paragraph(
                    ...
                    ParagraphStyle("cs_hdr", fontName="LG-Bold", fontSize=7.5,
        ...
        if not has_any:
            ...
            items.append(Paragraph(
                ...
                ParagraphStyle("njs", fontName="LG-Bold", fontSize=7,
...
def build_pattern_quick_review(pat: dict, qs: list) -> list:
    ...
    label_st = ParagraphStyle("qrl", fontName="LG-Bold", fontSize=7,
                               textColor=BLACK, leading=9, spaceBefore=4, spaceAfter=1)
    body_st  = ParagraphStyle("qrb", fontName="LG-Bold", fontSize=6.8,
                               textColor=GRAY_700, leading=9, leftIndent=6, spaceAfter=1)
    ...
    for q in qs:
        ...
        items.append(Paragraph(
            ...
            ParagraphStyle("qrt", fontName="LG-Bold", fontSize=7.5,
                           textColor=BLACK, leading=10, spaceBefore=6, spaceAfter=1)))
        ...
        if info:
            ...
            items.append(Paragraph(
                ...
                ParagraphStyle("nqr", fontName="LG-Bold", fontSize=6.8,
                               textColor=GRAY_500, leading=9)))
...
def build_inner_pdf(groups: list, sites: dict, doocs: dict):
    ...
    story.append(Paragraph("LeetMastery", ParagraphStyle(
        "brand", fontName="LG-Bold", fontSize=9, textColor=GRAY_500, alignment=TA_CENTER)))
    story.append(Spacer(1, 4))
    story.append(Paragraph("By Pattern", S["cover_title"]))
    story.append(Paragraph("Python Only · Print Edition · 9-up", ParagraphStyle(
        "sub2", fontName="LG-Bold", fontSize=10, textColor=GRAY_700,
        alignment=TA_CENTER, leading=13)))
    story.append(Spacer(1, 8))
    story.append(hr())
    story.append(Spacer(1, 5))
    story.append(Paragraph(
        f"{total_qs} questions · {len(groups)} patterns · bold black · 9-up",
        ParagraphStyle("ci", fontName="LG-Bold", fontSize=7,
                       textColor=GRAY_500, alignment=TA_CENTER)))
    story.append(Paragraph(
        "Descriptions + images · Brute force · All 4 sites · Sorted fewest→most",
        ParagraphStyle("ci2", fontName="LG-Bold", fontSize=7,
                       textColor=GRAY_500, alignment=TA_CENTER)))
    story.append(PageBreak())

    # ── Table of Contents ─────────────────────────────────────────────────────
    story.append(Paragraph("Contents", ParagraphStyle(
        "toch", fontName="LG-Bold", fontSize=10, textColor=BLACK, spaceAfter=4)))
    story.append(hr())
    ...
    for rank, (pat, qs) in enumerate(groups, 1):
        ...
        row = Table([[
            ...
            Paragraph(diff_str, ParagraphStyle(
                "dc", fontName="LG-Bold", fontSize=7, textColor=GRAY_500, alignment=TA_LEFT)),
        ...
        for q in qs:
            story.append(Paragraph(
                ...
                ParagraphStyle("tqe", fontName="LG", fontSize=6.5,
                               textColor=GRAY_700, leading=9)))
    story.append(PageBreak())
    ...
    for rank, (pat, qs) in enumerate(groups, 1):
        ...
        banner = Table([[Paragraph(
            ...
            ParagraphStyle("bnctr", fontName="LG-Bold", fontSize=12,
        ...
        story.append(Paragraph(
            ...
            ParagraphStyle("cbs", fontName="LG-Bold", fontSize=8,
                           textColor=GRAY_500, alignment=TA_CENTER)))
        story.append(PageBreak())
...
# ─── 9-up landscape imposer ───────────────────────────────────────────────────
def impose_9up_landscape(src_path: Path, dst_path: Path):
    ...
    for pg_idx in range(num_sheets):
        dst[pg_idx].insert_text(
            fitz.Point(L_W / 2 - 70, L_H - 5),
            f"Sheet {pg_idx + 1} / {num_sheets}  ·  LeetMastery By Pattern · Python Only · 9-up",
            fontsize=6, color=(0.5, 0.5, 0.5),
        )

    dst.save(str(dst_path), garbage=4, deflate=True)
    src.close(); dst.close()
    print(f"9-up landscape: {n} mini-pages → {num_sheets} sheets → {dst_path.name}")
...
if __name__ == "__main__":
    ...
    print("Imposing 9-up landscape…")
    impose_9up_landscape(INNER_PDF, OUTPUT_PDF)
